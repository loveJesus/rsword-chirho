...
//! zVerse4 compressed storage format implementation.
//!
//! zVerse4 is similar to zVerse but uses 12-byte index entries
//! (4-byte block_num + 4-byte offset + 4-byte size) to support larger entries.
...
use crate::byte_order_chirho::{ZBlockIndexChirho, ZVerse4IndexChirho, ReadSwordChirho};
...
/// zVerse4 compressed storage format with 12-byte index entries.
pub struct ZVerse4Chirho {
...
impl ZVerse4Chirho {
    ...
    fn extension_prefix_chirho(comp_type_chirho: CompressionTypeChirho) -> &'static str {
        match comp_type_chirho {
            ...
            CompressionTypeChirho::LzssChirho => "c",
            CompressionTypeChirho::NoneChirho => "c",
        }
    }

    /// Open an existing zVerse4 module.
    ...
    /// Open an existing zVerse4 module with specified compression type.
    ...
    /// Find the verse index entry (12-byte format).
    pub fn find_verse_index_chirho(
        &mut self,
        testament_chirho: u8,
        idx_off_chirho: u32,
    ) -> ResultChirho<ZVerse4IndexChirho> {
        let byte_offset_chirho = idx_off_chirho as u64 * ZVerse4IndexChirho::SIZE_CHIRHO as u64;
