...
//! Catholic versification system data.
//!
//! This contains the canonical book order and verse counts for the
//! Catholic versification system, which includes deuterocanonical books.

use crate::versification_chirho::{BookInfoChirho, VersificationChirho};

/// Create the Catholic versification system.
pub fn create_catholic_chirho() -> VersificationChirho {
    VersificationChirho::new_chirho("Catholic", create_ot_books_chirho(), create_nt_books_chirho())
}
...
fn create_ot_books_chirho() -> Vec<BookInfoChirho> {
    vec![
        ...
        BookInfoChirho::new_chirho("Esther", "Esth", "Esth", vec![
            ...
            // Additions to Esther (chapters 11-16 in Catholic Bibles)
            12, 6, 18, 19, 16, 24,
        ]),
        // I Maccabees (deuterocanonical)
        BookInfoChirho::new_chirho("I Maccabees", "1Macc", "1Mac", vec![
            64, 70, 60, 61, 68, 63, 50, 32, 73, 89,
            74, 53, 53, 49, 41, 24,
        ]),
        // II Maccabees (deuterocanonical)
...
mod tests_chirho {
    ...
    #[test]
    fn test_catholic_ot_books_chirho() {
        let ot_chirho = create_ot_books_chirho();
        // Catholic OT has 46 books (39 Protestant + 7 deuterocanonical)
        assert_eq!(ot_chirho.len(), 46);
        assert_eq!(ot_chirho[0].name_chirho, "Genesis");

        // Check deuterocanonical books exist
        assert!(ot_chirho.iter().any(|b_chirho| b_chirho.name_chirho == "Tobit"));
        assert!(ot_chirho.iter().any(|b_chirho| b_chirho.name_chirho == "Judith"));
        assert!(ot_chirho.iter().any(|b_chirho| b_chirho.name_chirho == "Wisdom"));
        assert!(ot_chirho.iter().any(|b_chirho| b_chirho.name_chirho == "Sirach"));
        assert!(ot_chirho.iter().any(|b_chirho| b_chirho.name_chirho == "Baruch"));
        assert!(ot_chirho.iter().any(|b_chirho| b_chirho.name_chirho == "I Maccabees"));
        assert!(ot_chirho.iter().any(|b_chirho| b_chirho.name_chirho == "II Maccabees"));
    }

    #[test]
    fn test_catholic_nt_books_chirho() {
        let nt_chirho = create_nt_books_chirho();
        assert_eq!(nt_chirho.len(), 27);
    }

    #[test]
    fn test_tobit_chapter_count_chirho() {
        let ot_chirho = create_ot_books_chirho();
        let tobit_chirho = ot_chirho.iter().find(|b_chirho| b_chirho.name_chirho == "Tobit").unwrap();
        assert_eq!(tobit_chirho.chapter_count_chirho, 14);
    }
}
