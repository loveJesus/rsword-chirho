...
//! Ethiopian Orthodox versification system data.
//!
//! The Ethiopian Orthodox Church has the largest biblical canon of any Christian
//! tradition, including books like 1 Enoch, Jubilees, and others not found in
//! Catholic or Protestant Bibles.
//!
//! Key features:
//! - Includes deuterocanonical books
//! - Includes unique books (1 Enoch, Jubilees, etc.)
//! - Different book order reflecting Ethiopian tradition

use crate::versification_chirho::{BookInfoChirho, VersificationChirho};

/// Create the Ethiopian Orthodox versification system.
pub fn create_ethiopian_chirho() -> VersificationChirho {
    VersificationChirho::new_chirho("Ethiopian", create_ot_books_chirho(), create_nt_books_chirho())
}

fn create_ot_books_chirho() -> Vec<BookInfoChirho> {
    // Ethiopian OT includes standard books plus deuterocanonical and unique books
    vec![
        // Pentateuch
        ...
        BookInfoChirho::new_chirho("1 Maccabees", "1Macc", "1Macc", vec![
            64, 70, 60, 61, 68, 63, 50, 32, 73, 89,
            74, 53, 53, 49, 41, 24,
        ]),
        BookInfoChirho::new_chirho("2 Maccabees", "2Macc", "2Macc", vec![
            36, 32, 40, 50, 27, 31, 42, 36, 29, 38,
            38, 46, 26, 46, 39,
        ]),
        BookInfoChirho::new_chirho("3 Maccabees", "3Macc", "3Macc", vec![
            29, 33, 30, 21, 51, 41, 23,
        ]),
        // Unique to Ethiopian canon
        // 1 Enoch (Henok)
        BookInfoChirho::new_chirho("1 Enoch", "1En", "1En", vec![
            ...
            17, 37, 11, 17, 9, 14, 9, 17, 6, 8,
            10, 20, 11, 6, 10, 6, 4, 3, 77, 42,
            19, 5, 14, 11, 7, 8, 6, 16, 16, 13,
            3, 3, 11, 13, 2,
        ]),
        // Jubilees
        BookInfoChirho::new_chirho("Jubilees", "Jub", "Jub", vec![
            29, 33, 35, 33, 19, 39, 39, 30, 15, 35,
            24, 31, 29, 24, 34, 31, 17, 19, 29, 13,
            26, 30, 32, 33, 23, 35, 27, 31, 20, 26,
            32, 35, 24, 19, 27, 18, 24, 14, 18, 13,
            28, 26, 16, 34, 16, 18, 12, 18, 23, 13,
        ]),
        // Poetry and Wisdom
...
fn create_nt_books_chirho() -> Vec<BookInfoChirho> {
    // Ethiopian NT includes standard 27 books plus some additional texts
    vec![
        ...
        BookInfoChirho::new_chirho("1 Corinthians", "1Cor", "1Cor", vec![
            31, 16, 23, 21, 13, 20, 40, 13, 27, 33,
            34, 31, 13, 40, 58, 24,
        ]),
        BookInfoChirho::new_chirho("2 Corinthians", "2Cor", "2Cor", vec![
        ...
        BookInfoChirho::new_chirho("1 Thessalonians", "1Thess", "1Thess", vec![
            10, 20, 13, 18, 28,
        ]),
        BookInfoChirho::new_chirho("2 Thessalonians", "2Thess", "2Thess", vec![
            12, 17, 18,
        ]),
        BookInfoChirho::new_chirho("1 Timothy", "1Tim", "1Tim", vec![
            20, 15, 16, 16, 25, 21,
        ]),
        BookInfoChirho::new_chirho("2 Timothy", "2Tim", "2Tim", vec![
            18, 26, 17, 22,
        ]),
        ...
        BookInfoChirho::new_chirho("1 Peter", "1Pet", "1Pet", vec![
            25, 25, 22, 19, 14,
        ]),
        BookInfoChirho::new_chirho("2 Peter", "2Pet", "2Pet", vec![
            21, 22, 18,
        ]),
        BookInfoChirho::new_chirho("1 John", "1John", "1John", vec![
            10, 29, 24, 21, 21,
        ]),
        BookInfoChirho::new_chirho("2 John", "2John", "2John", vec![
            13,
        ]),
        BookInfoChirho::new_chirho("3 John", "3John", "3John", vec![
            15,
        ]),
        BookInfoChirho::new_chirho("Jude", "Jude", "Jude", vec![
            25,
        ]),
        BookInfoChirho::new_chirho("Revelation", "Rev", "Rev", vec![
...
mod tests_chirho {
    ...
    #[test]
    fn test_ethiopian_ot_books_chirho() {
        let ot_chirho = create_ot_books_chirho();
        // Ethiopian OT has many more books than Protestant canon
        assert!(ot_chirho.len() > 39);
        assert_eq!(ot_chirho[0].name_chirho, "Genesis");
    }

    #[test]
    fn test_ethiopian_has_enoch_chirho() {
        let ot_chirho = create_ot_books_chirho();
        // Check for 1 Enoch (unique to Ethiopian canon)
        let enoch_chirho = ot_chirho.iter().find(|b| b.osis_chirho == "1En");
        assert!(enoch_chirho.is_some());
        // 1 Enoch has 108 chapters
        assert_eq!(enoch_chirho.unwrap().chapter_count_chirho, 105);
    }

    #[test]
    fn test_ethiopian_has_jubilees_chirho() {
        let ot_chirho = create_ot_books_chirho();
        // Check for Jubilees (unique to Ethiopian canon)
        let jubilees_chirho = ot_chirho.iter().find(|b| b.osis_chirho == "Jub");
        assert!(jubilees_chirho.is_some());
        // Jubilees has 50 chapters
        assert_eq!(jubilees_chirho.unwrap().chapter_count_chirho, 50);
    }

    #[test]
    fn test_ethiopian_nt_books_chirho() {
        let nt_chirho = create_nt_books_chirho();
        assert_eq!(nt_chirho.len(), 27);
        assert_eq!(nt_chirho[0].name_chirho, "Matthew");
    }

    #[test]
    fn test_ethiopian_creation_chirho() {
        let ethiopian_chirho = create_ethiopian_chirho();
        assert_eq!(ethiopian_chirho.name_chirho, "Ethiopian");
    }
}
