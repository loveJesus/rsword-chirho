...
//! KJV versification system data.
//!
//! This contains the canonical book order and verse counts for the
//! King James Version versification system.

use crate::versification_chirho::{BookInfoChirho, VersificationChirho};

/// Create the KJV versification system.
pub fn create_kjv_chirho() -> VersificationChirho {
    VersificationChirho::new_chirho("KJV", create_ot_books_chirho(), create_nt_books_chirho())
}
...
mod tests_chirho {
    ...
    #[test]
    fn test_kjv_ot_books_chirho() {
        let ot_chirho = create_ot_books_chirho();
        assert_eq!(ot_chirho.len(), 39);
        assert_eq!(ot_chirho[0].name_chirho, "Genesis");
        assert_eq!(ot_chirho[0].chapter_count_chirho, 50);
        assert_eq!(ot_chirho[38].name_chirho, "Malachi");
    }

    #[test]
    fn test_kjv_nt_books_chirho() {
        let nt_chirho = create_nt_books_chirho();
        assert_eq!(nt_chirho.len(), 27);
        assert_eq!(nt_chirho[0].name_chirho, "Matthew");
        assert_eq!(nt_chirho[0].chapter_count_chirho, 28);
        assert_eq!(nt_chirho[26].name_chirho, "Revelation of John");
    }

    #[test]
    fn test_verse_counts_chirho() {
        let ot_chirho = create_ot_books_chirho();

        // Genesis 1 has 31 verses
        assert_eq!(ot_chirho[0].max_verse_chirho(1), Some(31));

        // Psalms 119 has 176 verses
        assert_eq!(ot_chirho[18].max_verse_chirho(119), Some(176));
    }

    #[test]
    fn test_john_316_chirho() {
        let nt_chirho = create_nt_books_chirho();

        // John is the 4th NT book (index 3)
        let john_chirho = &nt_chirho[3];
        assert_eq!(john_chirho.name_chirho, "John");

        // John chapter 3 has 36 verses
        assert_eq!(john_chirho.max_verse_chirho(3), Some(36));
    }
}
