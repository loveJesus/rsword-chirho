...
//! NRSV versification system data.
//!
//! The NRSV (New Revised Standard Version) versification is used by NRSV
//! and similar modern ecumenical translations. It mostly follows the
//! Hebrew/Protestant verse numbering but with some differences.

use crate::versification_chirho::{BookInfoChirho, VersificationChirho};

/// Create the NRSV versification system.
pub fn create_nrsv_chirho() -> VersificationChirho {
    VersificationChirho::new_chirho("NRSV", create_ot_books_chirho(), create_nt_books_chirho())
}

fn create_ot_books_chirho() -> Vec<BookInfoChirho> {
    // NRSV OT follows Hebrew/Protestant canon with 39 books
    // Some verse counts differ from KJV
    vec![
...
mod tests_chirho {
    ...
    #[test]
    fn test_nrsv_creation_chirho() {
        let nrsv_chirho = create_nrsv_chirho();
        assert_eq!(nrsv_chirho.name_chirho, "NRSV");
    }
}
